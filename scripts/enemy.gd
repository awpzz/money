...
@onready var ambient_music = get_tree().current_scene.get_node("music")
@onready var chase_music = get_tree().current_scene.get_node("chase_music")
...
func update_target_location():
	if process_mode == Node.PROCESS_MODE_INHERIT and !killed:
		$NavigationAgent3D.target_position = destination.global_transform.origin

func compute_velocity(safe_velocity: Vector3) -> void :
	if process_mode == Node.PROCESS_MODE_INHERIT and destination != null and !killed:

...
func _ready() -> void :
	...
	set_button_setting("volumetricos", $volumetric_button)
	set_button_setting("brillo", $glow_button)
	set_button_setting("iluminacion", $ssil_button)
	set_button_setting("vsync", $vsync_button)
	set_option_setting("fps", $fps_button)
	set_option_setting("antialiasing", $aa_button)
	set_option_setting("sombras", $shadow_button)
	set_option_setting("modo_ventana", $window_mode)
	set_slider_setting("escala_3d", $scaling_slider)
	set_slider_setting("volumen_general", $volume_slider)
	set_slider_setting("volumen_efectos", $sfx_slider)
	set_slider_setting("volumen_musica", $music_slider)
	set_slider_setting("sensibilidad", $look_slider)
...
func set_button_setting(save_name, button):
	var loadd = FileAccess.open("user://" + save_name + ".ajustes", FileAccess.READ)
	if loadd:
		if loadd.get_as_text() == "false":
			button.button_pressed = false
		elif loadd.get_as_text() == "true":
...
func set_option_setting(save_name, button):
	var loadd = FileAccess.open("user://" + save_name + ".ajustes", FileAccess.READ)
	if loadd:
		button.selected = int(loadd.get_as_text())
		loadd.close()

func set_slider_setting(save_name, slider):
	var loadd = FileAccess.open("user://" + save_name + ".ajustes", FileAccess.READ)
	if loadd:
		slider.value = float(loadd.get_as_text())
		loadd.close()

func save_setting(save_name: String, value):
	var save = FileAccess.open("user://" + save_name + ".skibiditoilet", FileAccess.WRITE)
	save.store_string(str(value))
	save.close()

func set_volumetrics(toggled):
	save_setting("volumetricos", toggled)
	if can_play_sound:
		get_parent().get_node("interact").play()
	if environment != null:
		environment.volumetric_fog_enabled = toggled

func set_glow(toggled):
	save_setting("brillo", toggled)
	if can_play_sound:
		get_parent().get_node("interact").play()
	if environment != null:
...
func set_ssil(toggled):
	save_setting("iluminacion", toggled)
	if can_play_sound:
		get_parent().get_node("interact").play()
	if environment != null:
...
func scale_3d(value):
	save_setting("escala_3d", value)
	if can_play_sound:
		get_parent().get_node("interact").play()
	get_viewport().scaling_3d_scale = value

func set_master_volume(value):
	save_setting("volumen_general", value)
	if can_play_sound:
		get_parent().get_node("interact").play()
	AudioServer.set_bus_volume_db(0, linear_to_db(value))

func set_sfx_volume(value):
	save_setting("volumen_efectos", value)
	if can_play_sound:
		get_parent().get_node("interact").play()
	AudioServer.set_bus_volume_db(1, linear_to_db(value))

func set_music_volume(value):
	save_setting("volumen_musica", value)
	if can_play_sound:
		get_parent().get_node("interact").play()
	AudioServer.set_bus_volume_db(2, linear_to_db(value))

func set_look_speed(value):
	save_setting("sensibilidad", value)
	if can_play_sound:
		get_parent().get_node("interact").play()
	if player_head != null:
...
func set_window_mode(index):
	save_setting("modo_ventana", index)
	get_parent().get_node("interact").play()
	if index == 0:
